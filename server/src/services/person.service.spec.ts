...
describe(PersonService.name, () => {
  ...
  describe('mapFace', () => {
    ...
    it("should map the viewer's person from a shared face assignment", () => {
      const viewer = UserFactory.create();
      const viewerPerson = PersonFactory.create({ ownerId: viewer.id });
      const ownerPerson = PersonFactory.create();
      const face = AssetFaceFactory.from().person(ownerPerson).build();
      const linkedPerson = getForAssetFace(AssetFaceFactory.from().person(viewerPerson).build()).person!;

      expect(
        mapFaces({ ...getForAssetFace(face), people: [linkedPerson] }, AuthFactory.create({ id: viewer.id })).person,
      ).toEqual(mapPerson(viewerPerson));
    });
  });
});
